...
export function TorView() {
  ...
  const pull = useCallback(async () => {
    try {
      ...
      const list = (d.torSlots ?? d.slots ?? []).filter((s) => (s.kind || "tor") !== "openvpn");
      setSlots(list);
      setMgrUp(Boolean(d.procs?.slots));
    } catch {
      /* ignore */
    }
  }, []);
  ...
  async function slotAct(id: string, slotAction: "restart" | "stop") {
    ...
    try {
      const res = await relayFetch("/api/stack", {
        method: "POST",
        body: JSON.stringify({ slotAction, slotId: id, slotKind: "tor" }),
      });
      if (!res.ok) {
        toast.error(slotAction === "stop" ? "停用失败" : "重建失败");
        return;
      }
      toast.success(slotAction === "stop" ? `已停用 ${id}` : `正在重建 ${id}`);
      await pull();
    } finally {
      setBusyId(null);
    }
  ...
  async function doProbe(id: string) {
    ...
    try {
      const res = await relayFetch("/api/stack", {
        method: "POST",
        body: JSON.stringify({ probe: true, slotId: id, slotKind: "tor" }),
      });
  ...
  async function restartMgr() {
    const res = await relayFetch("/api/stack", {
      method: "POST",
      body: JSON.stringify({ action: "restart", target: "slots" }),
    });
    if (!res.ok) {
      toast.error("重启 Tor 管理器失败");
      return;
    }
    toast.success("正在重启 Tor 管理器");
    await pull();
  }
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-medium tracking-widest text-subtle uppercase">Tor</p>
          <h1 className="font-display mt-1 text-2xl font-medium tracking-tight">Tor 电路</h1>
          <p className="mt-1 text-sm text-muted">
            本机 SOCKS `127.0.0.1:905x`。停用会写 DISABLED，管理器循环内不会再拉起。
          </p>
        </div>
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 font-mono text-xs text-muted">
                  SOCKS {port || "—"} · IP {s.egress_ip || "—"} · pid {s.pid || 0}
